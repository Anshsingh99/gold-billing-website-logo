  ...
  const handleSaveBill = async () => {
    ...
    try {
      ...
      if (isEditing && billId) {
        ...
        const { id, bill_date, user_id, bill_items, customers, ...updateData } =
          billData as any;
        ...
        const remaining =
          (billDataForInsert.total_amount || 0) -
          (billDataForInsert.paid_amount || 0);
          ...
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setBillData({ ...billData, bill_number: e.target.value })
                }
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                value={customers
                  ...
                  .find((option) => option.value === billData.customer_id)}
        ...
        <CardContent>
          <div className="space-y-2 sm:space-y-4 overflow-x-auto">
            {billItems.map((item, index) => (
              <div
                key={index}
                className="grid gap-2 sm:gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 p-2 sm:p-4 border border-amber-100 dark:border-amber-900 rounded"
              >
                <div className="space-y-2">
                  <Label className="text-amber-700 dark:text-amber-400 text-xs">
                    Product
                  </Label>
                  <Select
                    ...
                    value={
                      productOptions.find(
                        (option: any) =>
                          String(option.value) === String(item.product_id),
                      ) || null
                    }
                    onChange={(selected) =>
                      updateBillItem(index, "product_id", selected?.value || "")
                    }
                    className="text-sm"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={item.rate}
                    onChange={(e) =>
                      updateBillItem(index, "rate", parseFloat(e.target.value))
                    }
                    className="border-amber-200 dark:border-amber-800 text-sm"
                  />
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="border-t border-amber-200 dark:border-amber-800 pt-4">
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">
                    ₹{billData.total_amount?.toFixed(2) || "0.00"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Amount Paid:</span>
                  <span className="font-semibold">
                    ₹{(billData.paid_amount || 0).toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between text-lg border-t border-amber-200 dark:border-amber-800 pt-2 text-amber-900 dark:text-amber-300">
                  <span className="font-bold">Remaining Balance:</span>
                  <span className="font-bold">
                    ₹{(billData.remaining_amount || 0).toFixed(2)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <Label
                  htmlFor="installment"
                  className="text-xs sm:text-sm text-amber-700 dark:text-amber-400 cursor-pointer"
                >
                  Enable Installments
                </Label>
              </div>
