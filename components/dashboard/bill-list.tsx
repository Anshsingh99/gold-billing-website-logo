...
export default function BillList({
  ...
  const handleDownloadPDF = async (bill: Bill) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to download bill',
        variant: 'destructive',
      })
    } finally {
  ...
  const handleSendWhatsApp = async (bill: Bill) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to send WhatsApp',
        variant: 'destructive',
      })
    } finally {
  ...
  return (
    <Card className="border-amber-200 dark:border-amber-800">
      <CardHeader>
        <CardTitle className="text-sm sm:text-base text-amber-900 dark:text-amber-300">Your Bills</CardTitle>
        <CardDescription className="text-xs sm:text-sm">View and manage all your bills</CardDescription>
      </CardHeader>
      <CardContent>
        {bills.length > 0 && (
            ...
            <table className="w-full text-xs sm:text-sm">
              ...
              <tbody>
                {filteredBills.map((bill) => (
                  <tr key={bill.id} className="border-b border-amber-100 dark:border-amber-900">
                    <td className="py-2 sm:py-3 px-2 sm:px-4 font-semibold text-xs sm:text-sm">{bill.bill_number}</td>
                    <td className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm hidden sm:table-cell">{bill.customers?.name || 'Unknown'}</td>
                    ...
                    <td className="py-2 sm:py-3 px-1 sm:px-4 text-right">
                      <div className="flex flex-col sm:flex-row gap-1 sm:gap-1">
                        ...
                        <Button
                          ...
                          title={downloadingId === bill.id ? 'Downloading...' : 'Download PDF'}
                        >
                        ...
                        <Button
                          ...
                          title={whatsappSendingId === bill.id ? 'Sending...' : 'Send WhatsApp'}
                        >
